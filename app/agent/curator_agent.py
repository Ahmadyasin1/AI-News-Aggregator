...
{pref_text}"""
    ...
    def rank_digests(self, digests: List[dict]) -> List[RankedArticle]:
        ...
        digest_list = "\n\n".join([
            f"id: {d['id']} title: {d['title']} summary: {d['summary']} type: {d['article_type']}"
            for d in digests
        ])

        prompt = (
            f"{self.system_prompt}\n\n"
            f"Rank these {len(digests)} AI digests by relevance (0.0-10.0) and give rank (1-{len(digests)})."
            f" Output JSON as list of objects with digest_id, relevance_score, rank, reasoning.\n\n"
            f"Digests:\n{digest_list}\n"
            f"Respond only with JSON array."
        )

        text = self.generate_text(prompt, max_tokens=500, temperature=0.2)
        ...
        # Fallback: simple heuristic by length
        fallback = []
        for i, d in enumerate(digests, start=1):
            fallback.append(RankedArticle(
                ...
                relevance_score=5.0,
                rank=i,
                reasoning="Fallback static rank"
            ))
        return fallback
