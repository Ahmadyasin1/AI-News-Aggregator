...
class EmailAgent(BaseAgent):
    ...
    def generate_introduction(self, ranked_articles: List) -> EmailIntroduction:
        ...
        prompt = (
            f"{EMAIL_PROMPT}\n\n"
            f"Create JSON with keys greeting and introduction.\n"
            f"User: {self.user_profile['name']}\nDate: {current_date}\nTop articles:\n{article_summaries}\n"
            f"Respond only with JSON."
        )

        text = self.generate_text(prompt, max_tokens=250, temperature=0.7)
        parsed = self.parse_json_output(text)
        if isinstance(parsed, dict):
            greeting = parsed.get("greeting") or f"Hey {self.user_profile['name']}, here is your daily digest of AI news for {current_date}."
            introduction = parsed.get("introduction") or "Here are the top AI news articles for today."
            return EmailIntroduction(greeting=greeting.strip(), introduction=introduction.strip())
